...
class World(object):
    ...
    def restart(self):
        ...
        if self.vehicle is not None:
            ...
            # spawn_point = carla.Transform(carla.Location(x=14.01, y= 306.40,z=1.37))
        ...
        while self.vehicle is None:
            ...
            spawn_point_town2 = carla.Transform(carla.Location(x=14.01, y= 306.40,z=1.37))
            spawn_point_town1 = carla.Transform(carla.Location(x=396.4, y= 294.80,z=40.0),carla.Rotation(yaw = -90))

            stop_go = carla.Transform(carla.Location(x=-2.9, y= -182.9,z=3.37),carla.Rotation(yaw = 90))
            # spawn_point = random.choice(spawn_points)
            self.vehicle = self.world.spawn_actor(blueprint, stop_go)
...
class TrafficDetector(object):
    ...
    def __init__(self, vehicle, target_waypoint, sign_distance, vehicle_distance):
        ...
        self._speedlimit_history.append(40.0)
    ...
    def define_hazard_vehicle(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,18.0):        
                return (True,target_vehicle)
        return (False,None)
    ...
    def get_traffic_light(self):
        ...
        if world.map_name == 'Town01' or world.map_name == 'Town02':
            return self._get_light_europe_style()
        else:
            return self._get_light_us_style()

    def _get_light_europe_style(self):
        """
...
class GlobalRoutePlanner(object):
    ...
    def build_graph(self):
        ...
        nx.write_multiline_adjlist(graph,str(time.strftime('%Y.%m.%d_%H%M%S',time.localtime(time.time())))+".multiline_adjlist")
        return graph, id_map
...
class ACC_Controller(object):
    ...
    def __init__(self,vehicle, opt_dict={}):
        ...
        self._traffic_detector = TrafficDetector(self._vehicle,self._target_waypoint,12.5,150.0)
    ...
    def emergency_brake_situation(self):
        ...
        light_state, traffic_light = self._traffic_detector.get_traffic_light()
    ...
    def set_target_distance(self):
        ...
        if  front_vehicle_state:
            frontspeed=get_speed(front_vehicle)
            # print("frontspeed",frontspeed)
            Egovehicle_speed=get_speed(self._vehicle)
            # print("Egovehicle_speed",Egovehicle_speed)
            RelaSpeed=Egovehicle_speed-frontspeed
            # print("RelaSpeed",RelaSpeed)
            ap=front_vehicle.get_acceleration()
            AP=math.sqrt(ap.x**2+ap.y**2+ap.z**2)
            th= t0 - Cv * RelaSpeed - Ca * AP
            if th>th_max:
                return th_max* RelaSpeed
            elif th<th_min:
                return th_min * RelaSpeed
            else:
                return th*RelaSpeed
...
class Recorder(object):
    def __init__(self,vehicle,controller,workbook):
        ...
        self.sheet.write(0,23,"Front_Target")       
        self.sheet.write(0,24,"Traffic_Light_State")
        self.sheet.write(0,25,"Speed_Limit_State")

        self.sheet.write(0,26,"Target_location_x")
        self.sheet.write(0,27,"Target_location_y")
    ...
    def start_recorder(self):
        ...
        worksheet.write(row,26, target_location_x)
        worksheet.write(row,27, target_location_y)
        ...
        if front_vehicle_state:
            ...
            worksheet.write(row,23,str(front_vehicle.id))
            

        speed_limit = controller._traffic_detector.get_speed_limit()
        speed_limit_value = 40.0 if speed_limit == "Init" else float(speed_limit)
        traffic_light_state, traffic_light = controller._traffic_detector.get_traffic_light()
        ...
        worksheet.write(row,24,light_value)
        worksheet.write(row,25,speed_limit_value)
...
def game_loop(args):
    ...
    try:
        ...
        transform_2 = carla.Transform(carla.Location(x=-3.42, y= -162.5,z=2.37),carla.Rotation(yaw = 90))
        ...
        # ACC_controller.set_destination((131.9, 55.40, 40.0)) # Town 01
        # ACC_controller.set_destination((173.78, 109.43, 1.37)) # Town 02
