...
class TrafficDetector(object):
    ...
    def search_rear_vehicle_left_lane(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            loc = target_vehicle.get_location()
            v_rear = get_speed(target_vehicle) / 3.6
            v_ego = get_speed(vehicle) / 3.6
            s_critical = (v_rear - v_ego) * t_b + (v_rear - v_ego)**2 / (2*a) + v_ego * t_g

            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        180.0 + vehicle.get_transform().rotation.yaw,s_critical):        
                return (True,target_vehicle)
        return (False,None)  
    ...
    def search_front_vehicle_left_lane(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,20.0):        
                # print (target_vehicle.id == vehicle_list[0])
                return (True,target_vehicle)
        return (False,None)    
    ...
    def define_hazard_vehicle(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,40.0):        
                return (True,target_vehicle)
        return (False,None)
...
class World(object):
    ...
    def restart(self):
        ...
        blueprint.set_attribute('color', '241,158,194')
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    self._controller.preset_speed = 70.0
                    world.hud.notification("Cruise speed set to 70.0 km/h.")
                elif event.key == K_F8:
                    self._controller.preset_speed = 80.0
                    world.hud.notification("Cruise speed set to 80.0 km/h.")
                elif event.key == K_F6:
                    self._controller.preset_speed = 60.0
                    world.hud.notification("Cruise speed set to 60.0 km/h.")
                elif event.key == K_F5:
                    self._controller.preset_speed = 50.0
                    world.hud.notification("Cruise speed set to 50.0 km/h.")
                elif event.key == K_F4:
                    self._controller.preset_speed = 40.0
                    world.hud.notification("Cruise speed set to 40.0 km/h.")
                elif event.key == K_F9:
                    self._controller.left_lane_change(get_speed(self._vehicle) * 2.2 if get_speed(self._vehicle) > 50.0 else 50.0)  
                    world.hud.notification("Left Automatic Lane Change Activated.")
                elif event.key == K_F10:
                    self._controller.right_lane_change(get_speed(self._vehicle) * 2.2 if get_speed(self._vehicle) > 50.0 else 50.0)
                    world.hud.notification("Right Automatic Lane Change Activated.")
...
class ACC_Controller(object):
    ...
    def search_overtake_vehicle(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,45.0):        
                return (True,target_vehicle)
        return (False,None)
    ...
    def set_target_speed(self):
        ...
        if overtake_vehicle_state is True:
            if (get_speed(overtake_vehicle) < self.preset_speed * 0.85) and  (self.emergency_brake_situation() is False):
                self._front_overtake_target = overtake_vehicle
                self._overtake_intention = True
        
        if vehicle_state is True and overtake_vehicle_state is False:
    ...
    def left_lane_change(self,inject_distance):
        ...
        if rear_left_vehicle_state is False and front_left_vehicle_state is False:
            ...
            new_waypoint = self._map.get_waypoint(carla.Location(self._vehicle.get_location().x + inject_distance, 
    ...
    def search_rear_overtake_target(self):
        threshold = 150.0
        vehicle = self._vehicle
        vehicle_list = self._vehiclelist
        current_map = self._map  
    ...
    def decide_on_overtake(self):    
        ...
        if self._overtake_intention is True:    
            ...
            if self._first_move_done is True and self._overtake_rightchange is False and self._overtake_done is False:
                _ , self._rear_overtake_target = self.search_rear_overtake_target()
    ...
    def initialize_PID_controller(self, opt_dict):
        ...
        self._sampling_radius = self._target_speed * 0.6 / 3.6  # 0.5 seconds horizon
...
class Recorder(object):
    def __init__(self,vehicle,controller,workbook):
        ...
        self.sheet.write(0,29,"Left_rear_car_x")
        self.sheet.write(0,30,"Left_rear_car_y")
        self.sheet.write(0,31,"Left_rear_car_speed")
        self.sheet.write(0,32,"Left_rear_car_S_critical")
        self.sheet.write(0,33,"Left_rear_car_relative_distance")

        self.sheet.write(0,34,"Right_rear_car_x")
        self.sheet.write(0,35,"Right_rear_car_y")
        self.sheet.write(0,36,"Right_rear_car_speed")
        self.sheet.write(0,37,"Right_rear_car_S_critical")
        self.sheet.write(0,38,"Right_rear_car_relative_distance")
    ...
    def start_recorder(self):
        ...
        if rear_right_vehicle_state:
            ...
            rr_s = (rr_speed / 3.6 - ego_speed/3.6) * t_b + (rr_speed / 3.6 - ego_speed/3.6)**2 / (2*a) + ego_speed * t_g / 3.6
        ...
        if rear_left_vehicle_state:
            ...
            rl_s = (rl_speed / 3.6 - ego_speed/3.6) * t_b + (rl_speed / 3.6 - ego_speed/3.6)**2 / (2*a) + ego_speed * t_g / 3.6
        ...
        if front_right_vehicle_state:
            ...
            worksheet.write(row,39,fr_location_x)
            worksheet.write(row,40,fr_location_y)
            worksheet.write(row,41,fr_speed)
            worksheet.write(row,42,fr_distance)
        ...
        if front_left_vehicle_state:
            ...
            worksheet.write(row,43,fl_location_x)
            worksheet.write(row,44,fl_location_y)
            worksheet.write(row,45,fl_speed)
            worksheet.write(row,46,fl_distance)
...
def game_loop(args):
    ...
    try:
        ...
        transform_5 = carla.Transform(carla.Location(x=-2450 ,y=12.25, z=0.40), carla.Rotation(pitch=0, yaw=0, roll=0))
        ...
        color_1 = carModel_1.get_attribute('color').recommended_values[1]
        carModel_1.set_attribute('color',color_1)
        ...
        carModel_2 = world.world.get_blueprint_library().find('vehicle.audi.tt')
        color_2 = carModel_2.get_attribute('color').recommended_values[6]
        carModel_2.set_attribute('color',color_2)
        carActor_2 = world.world.try_spawn_actor(carModel_2,transform_5)
        carActor_2.set_autopilot (True)        
        supporting_actor_list.append(carActor_2)   
        ...
        controller = KeyboardControl(world, False,ACC_controller)
