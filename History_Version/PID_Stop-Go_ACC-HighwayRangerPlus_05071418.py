...
class ACC_Controller(object):
    ...
    def left_lane_change(self):
        ...
        new_waypoint = self._map.get_waypoint(carla.Location(self._vehicle.get_location().x + 67.0, 
    ...
    def right_lane_change(self):
        self._waypoints_queue.clear()
        new_waypoint = self._map.get_waypoint(carla.Location(self._vehicle.get_location().x + 170.0, 
                                                                self._vehicle.get_location().y + 3.5,
                                                                self._vehicle.get_location().z)) 
    ...
    def decide_on_overtake(self):    
        ...
        if self._overtake_intention is True:    
            ...
            if self._overtake_target is not None and self._overtake_leftchange is False and rear_left_vehicle_state is False \
                and front_left_vehicle_state is False and self._overtake_rightchange is False and self._overtake_done is False:
            ...
            if self._overtake_leftchange is True and self._overtake_rightchange is False:
                _ , self._overtake_target = self.refresh_overtake_target()
                threshold_distance = self._vehicle.get_location().x - self._overtake_target.get_location().x if self._overtake_target is not None else 0.0
                # print("overtake target == None is", self._overtake_target == None)
            
            if self._overtake_target is not None:
                if (threshold_distance >= 20.0 and front_right_vehicle_state is False and self._overtake_leftchange is True \
                ...
                if (rear_right_vehicle_state is False and self._overtake_leftchange is True and \
            ...
            if self._overtake_leftchange is True and self._overtake_rightchange is True and self._overtake_done is False:
                if self._map.get_waypoint(self._vehicle.get_location()).lane_id == self._lane_id - 1:
                    self._lane_id = self._map.get_waypoint(self._vehicle.get_location()).lane_id               
                    self._overtake_done = True

            
            if self._overtake_leftchange is True and self._overtake_rightchange is True and self._overtake_done is True:
                self._overtake_intention = False
                self._overtake_target = None
                self._overtake_leftchange = False
                self._overtake_rightchange = False
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            print (carActor_1.get_acceleration().x)
            # print (way_point)
