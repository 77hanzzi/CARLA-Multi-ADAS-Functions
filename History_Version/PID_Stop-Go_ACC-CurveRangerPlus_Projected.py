...
class TrafficDetector(object):
    ...
    def __init__(self, vehicle, target_waypoint, sign_distance, vehicle_distance):
        ...
        self.rr_s_critical = 10.0
        self.rl_s_critical = 10.0
    ...
    def search_rear_vehicle_left_lane(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            self.rl_s_critical = (v_rear - v_ego) * t_b + (v_rear - v_ego)**2 / (2*a) + v_ego * t_g

            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        180.0 + vehicle.get_transform().rotation.yaw, self.rl_s_critical):        
                return (True,target_vehicle)
        return (False,None)  
    ...
    def search_rear_vehicle_right_lane(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            v_rear = get_speed(target_vehicle) / 3.6
            v_ego = get_speed(vehicle) / 3.6
            self.rr_s_critical = (v_rear - v_ego) * t_b + (v_rear - v_ego)**2 / (2*a) + v_ego * t_g

            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        180.0 + vehicle.get_transform().rotation.yaw,self.rr_s_critical):        
                return (True,target_vehicle)
        return (False,None)       
...
class World(object):
    ...
    def restart(self):
        ...
        while self.vehicle is None:
            ...
            follow_test = carla.Transform(carla.Location(x=-2415 ,y=12.25, z=0.40), carla.Rotation(pitch=0, yaw=0, roll=0))
            curve_test = spawn_points[0]
            frequent_right_change = carla.Transform(carla.Location(x=-2420 ,y=5.8, z=0.40), carla.Rotation(pitch=0, yaw=0, roll=0))
            frequent_left_change = carla.Transform(carla.Location(x=-2420 ,y=15.25, z=3.40), carla.Rotation(pitch=0, yaw=0, roll=0))
            ...
            self.vehicle = self.world.try_spawn_actor(blueprint,  curve_test)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    self._controller.preset_speed = 90.0
                    world.hud.notification("Cruise speed set to 90.0 km/h.")
                elif event.key == K_F4:
                    self._controller.preset_speed = 40.0
                    world.hud.notification("Cruise speed set to 40.0 km/h.")
                elif event.key == K_F9:
                    # self._inject_factor += 0.1
                    self._controller.left_lane_change(get_speed(self._vehicle) * self._inject_factor if get_speed(self._vehicle) > 45.0 else 50.0)  
                    world.hud.notification("Left Automatic Lane Change Activated.")
                    print (self._inject_factor)
                elif event.key == K_F10:
                    # self._inject_factor += 0.1
                    self._controller.right_lane_change(get_speed(self._vehicle) * self._inject_factor if get_speed(self._vehicle) > 45.0 else 50.0)
...
class ACC_Controller(object):
    ...
    def __init__(self,vehicle, opt_dict={}):
        ...
        self._traffic_detector = TrafficDetector(self._vehicle,self._target_waypoint,12.5,100.0)
    ...
    def search_overtake_vehicle(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,80.0):  
    ...
    def initialize_PID_controller(self, opt_dict):
        ...
        self._sampling_radius = self._target_speed * 0.4 / 3.6  # 0.5 seconds horizon
        ...
        self._compute_next_waypoints(k=60)        
    ...
    def run_step(self, debug=True):
        ...
        if len(self._waypoints_queue) < int(self._waypoints_queue.maxlen * 0.5):
                self._compute_next_waypoints(k=5)
...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            ...
            carla.Transform(carla.Location(x=0.15,y=-0.3, z=1.3), carla.Rotation(pitch=5)),
            carla.Transform(carla.Location(x=-5.5, z=2.8), carla.Rotation(pitch=-15)),
            carla.Transform(carla.Location(x=0.2, z=1.6), carla.Rotation(pitch=0)),
            carla.Transform(carla.Location(x=-1.5, z=1.4), carla.Rotation(yaw=180)),
...
def game_loop(args):
    ...
    try:
        ...
        carActor_2 = world.world.try_spawn_actor(carModel_2,transform_6)
