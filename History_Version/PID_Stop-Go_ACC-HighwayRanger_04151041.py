...
class TrafficDetector(object):
    ...
    def record_lane_id(self):
        ...
        selfs._lane_history.append(ego_vehicle_waypoint.lane_id)
...
class ACC_Controller(object):
    ...
    def __init__(self,vehicle, opt_dict={}):
        ...
        self._traffic_detector = TrafficDetector(self._vehicle,self._target_waypoint,12.5,170.0)
    ...
    def refresh_sampling_radius(self):
        if get_speed(self._vehicle) <= 15.0:
            self._sampling_radius = 2.0  # 0.5 seconds horizon

        elif get_speed(self._vehicle)>15.0 and get_speed(self._vehicle) <= 45.0:
            self._sampling_radius = get_speed(self._vehicle) * 0.5 / 3.8  # 0.5 seconds horizon
        elif get_speed(self._vehicle) > 45.0 and get_speed(self._vehicle) <= 65.0:
            self._sampling_radius = get_speed(self._vehicle) * 0.5 / 3.2
        elif get_speed(self._vehicle) > 65.0:
            self._sampling_radius = 12.0
    ...
    def refresh_target_speed(self):
        vehicle_state, front_vehicle = self._traffic_detector.search_front_vehicle()                
        if vehicle_state:
            # print (get_speed(front_vehicle))
            if get_speed(front_vehicle) < self.preset_speed * 0.8:
                self.right_lane_change()
            self._target_speed = get_speed(front_vehicle)
        self._target_speed = self.preset_speed
    ...
    def left_lane_change(self):
        ...
        if vehicle_state is False:
            ...
            control.throttle = 0.37
            self._vehicle.apply_control(control)
            self._waypoints_queue.clear()
            new_waypoint = self._map.get_waypoint(carla.Location(self._vehicle.get_location().x + 18.0, 
                                                                    self._vehicle.get_location().y - 3.0,
    ...
    def run_step(self, debug=True):
        ...
        self.refresh_sampling_radius()
        self.refresh_target_speed()
        ...
        if not self._waypoint_buffer:
            for i in range(self._buffer_size):
                if self._waypoints_queue:
                    ...
                    break
        self._sampling_radius = get_speed(self._vehicle) * 0.5 / 4.0  # 0.5 seconds horizon
        # print(self._sampling_radius)
