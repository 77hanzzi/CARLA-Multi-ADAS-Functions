...
class TrafficDetector(object):
    ...
    def record_lane_id(self):
        ...
        selfsd._lane_history.append(ego_vehicle_waypoint.lane_id)
    ...
    def search_rear_vehicle_left_lane(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            target_vehicle_waypoint =  current_map.get_waypoint(target_vehicle.get_location())
            if target_vehicle_waypoint.road_id != ego_vehicle_waypoint.road_id or \
                            target_vehicle_waypoint.lane_id != ego_vehicle_waypoint.lane_id + 1:
                continue

            loc = target_vehicle.get_location()

            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        180.0 + vehicle.get_transform().rotation.yaw,15.0):        
                # print (target_vehicle.id == vehicle_list[0])
                return (True,target_vehicle)
        return (False,None)    
    ...
    def define_hazard_vehicle(self):
        ...
        for target_vehicle in vehicle_list:
            ...
            if is_within_distance_ahead(loc,ego_vehicle_location,
                                        vehicle.get_transform().rotation.yaw,35.0):        
                return (True,target_vehicle)
        return (False,None)
...
class World(object):
    ...
    def restart(self):
        ...
        while self.vehicle is None:
            ...
            spawn_point_2 = carla.Transform(carla.Location(x=-2435 ,y=12.25, z=0.40), carla.Rotation(pitch=0, yaw=0, roll=0))
...
class ACC_Controller(object):
    ...
    def __init__(self,vehicle, opt_dict={}):
        ...
        self._traffic_detector = TrafficDetector(self._vehicle,self._target_waypoint,12.5,60.0)
    ...
    def decide_on_overtake(self):    
        ...
        if self._overtake_intention is True:             
            right_vehicle_state, rear_vehicle_right_lane = self._traffic_detector.search_rear_vehicle_right_lane()
            
            left_vehicle_state, rear_vehicle_left_lane = self._traffic_detector.search_rear_vehicle_left_lane()
            
            if left_vehicle_state:

                print (rear_vehicle_left_lane.type_id)

            if self._overtake_target is not None:
                threshold_distance = self._vehicle.get_location().x - self._overtake_target.get_location().x
            
            while self._overtake_target is not None and self._overtake_leftchange is False and left_vehicle_state is False:
            ...
            while self._overtake_target is not None and threshold_distance >= 20.0 and right_vehicle_state is False and \
                                            self._overtake_leftchange is True and self._overtake_rightchange is False:
    ...
    def run_step(self, debug=True):
        ...
        # if self._overtake_target is not None:
        #     print (self._overtake_target.get_location())
        ...
        # vehicle_state, vehicle = self._traffic_detector.define_hazard_vehicle()
        # if vehicle_state:
        #     hazard_detected = True        
        
        # print (hazard_detected)
        # print (self.emergency_brake_situation())
