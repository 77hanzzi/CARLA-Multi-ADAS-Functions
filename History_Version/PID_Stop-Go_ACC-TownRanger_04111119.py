...
class ACC_Controller(object):
    ...
    def __init__(self,vehicle, opt_dict={}):
        ...
        # SETTING controller
        self.preset_speed = 25.0
        self.set_PID_controller(opt_dict)

        self._traffic_detector = TrafficDetector(self._vehicle,self._target_waypoint,12.5,25.0)
    ...
    def refresh_sampling_radius(self):
        if get_speed(self._vehicle) <= 15.0:
            ...
            self._sampling_radius = get_speed(self._vehicle) * 0.5 / 3.2  # 0.5 seconds horizon
        elif get_speed(self._vehicle) > 45.0 and get_speed(self._vehicle) <= 65.0:
            self._sampling_radius = get_speed(self._vehicle) * 0.5 / 2.8
    ...
    def targetspeed_setting(self):
        ...
        if vehicle_state is True:
            return get_speed(target_vehicle) - 2.0
        else:
            if self._traffic_detector.get_speed_limit() != 'Init':
                return float(self._traffic_detector.get_speed_limit())
